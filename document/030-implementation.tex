...
We will use statistical and landmarking meta-features:
\begin{description}
    \item[Statistical] meta-features are computed from the dataset
        characteristics: number of instances, number of features, number of
        classes, ratio of instances to features, etc.

    \item[Landmarking] meta-features are computed by training a set of simple
        models on the dataset and using their performance as meta-features.
        These models are called \emph{landmarking} models and are usually simple
        and fast to train. The models used in this work are:
        \begin{itemize}
            % TODO: fill this properly
            \item \emph{ZeroR}: A model that always predicts the most frequent
                  class.
            \item \emph{OneR}: A model that uses a single feature to predict the
                  class.
            \item \emph{DecisionStump}: A decision tree with a single split.
            \item \emph{NaiveBayes}: A Naive Bayes classifier.
            \item \emph{RandomNode}: A decision tree with a single split on a
                  random feature.
            \item \emph{RandomTree}: A decision tree with a random split.
            \item \emph{RandomGuess}: A model that predicts a random class.
        \end{itemize}
\end{description}
...
\emph{PyCall.jl} package, which allows us to use \emph{PyMFE} from Julia.





\cite{JMLR:v21:19-348,WelcomePyMFEDocumentation}
