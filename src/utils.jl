...
"""
normalize a kernel function K(x, y, ...)
"""
...
"Normalized version of the asin kernel"
...
kernel_acos(x, y, σ=1.0, n::Integer=1) =
    let
        fun = if n == 0
            kernel_acos_0
        elseif n == 1
...
"""
Safe arccosine function that returns 0 if x > 1 and π if x < -1 to prevent NaNs
"""
safe_acos(x) =
    if x > 1
        0
